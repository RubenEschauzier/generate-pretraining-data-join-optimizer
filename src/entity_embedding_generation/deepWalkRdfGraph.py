...
def load_graph(datasetLocation):
    g = Graph()
    g.parse(datasetLocation, format="nt")
    return g
...
def start_rdf_predicate_2_vec(num_sim_pred, num_sim_subj, num_sim_obj, depth_walk, datasetLocation, outputLocation,
                              save_model=True):
    g = load_graph(datasetLocation)
    ...
    if save_model:
        save_rdf_predicate_model(model, predicates, outputLocation)


if __name__ == "__main__":
    depth = 1
    # Hardcoded location of system-wide instantiation of benchmark used. To reproduce point to your own location
    instantiation_benchmark_location = r"C:\Users\Administrator\projects\benchmarks\watdiv-dataset\dataset.nt"
    vector_save_location = r"C:\Users\Administrator\projects\preprocess_data_rl_optimizer\dataWatDiv\vectorsWatDiv" \
                           r"\vectors_depth_{}_test.txt"

    start_rdf_predicate_2_vec(1000, 100, 100, depth, instantiation_benchmark_location,
                              vector_save_location.format(depth),
                              True)
